...
class CategoryDao  with HasDatabase, DbTable{
  ...
  Future<int> update(CategoryPo widget) async {
    ...
    return await database.transaction((tran) async =>
  ...
  Future<bool> existWidgetInCollect(int categoryId, int widgetId) async {
    ...
    List<Map<String, dynamic>> rawData = await database.rawQuery(sql, [categoryId, widgetId]);
    if (rawData.isNotEmpty) {
      return rawData[0]['count'] > 0;
    }
    return false;
